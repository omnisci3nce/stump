	...
	async fn run(&mut self, ctx: RunnerCtx) -> CoreResult<()> {
		...
		while running {
			tokio::select! {
				...
				// using `emit_event`, the lag would build up and then ONLY send the first and second
				...
				// 		core_ctx
				// 			.emit_event(CoreEvent::JobProgress(progress));
				// 	} else {
				...
				job_result = &mut job_fn => {
					...
					if let Err(err) = job_result {
						core_ctx.emit_event(CoreEvent::JobFailed {
...
pub async fn persist_job_start(
	...
	ctx.emit_event(CoreEvent::job_started(
		id.clone(),
		0,
		task_count,
